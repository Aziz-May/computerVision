...
# ---------------------------
# PROCESS VIDEO WITH MOG
# ---------------------------
def process_video_with_mog(video_path, class_name, video_name, crop_percent=0.25):
    """
    Process entire video with background subtraction focused on center
    """
    ...
    while True:
        if not paused:
            ...
            display = np.hstack([cropped_frame, fg_mask_bgr])
            
            # Add text overlay
            cv2.putText(display, f"Frame: {frame_count}/{total_frames}", (10, 30),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            cv2.putText(display, f"{class_name}", (10, 60),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            
            # Show the result
            cv2.imshow('Original (cropped) | Background Subtraction', display)
...
if __name__ == "__main__":
    ...
    if video_path:
        process_video_with_mog(video_path, class_name, video_name)
    else:
